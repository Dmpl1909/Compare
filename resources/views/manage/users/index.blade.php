...
<div class="space-y-8">
    ...
    <div class="relative overflow-hidden rounded-2xl border border-purple-500/20 bg-gradient-to-br from-purple-900/30 via-blue-900/30 to-slate-900/50 p-8 backdrop-blur-sm shadow-xl shadow-purple-500/10">
        ...
        <div class="relative flex items-start justify-between gap-4 flex-col sm:flex-row">
            <div>
                <div class="mb-2 inline-block rounded-full bg-purple-500/20 px-3 py-1 text-xs font-semibold text-purple-300 backdrop-blur-sm">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-800/50 bg-gradient-to-br from-slate-900/50 to-slate-900/80 backdrop-blur-sm shadow-xl">
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="border-b border-slate-800/50 bg-gradient-to-r from-purple-900/20 to-blue-900/20">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-purple-300">Utilizador</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-purple-300">Email</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-purple-300">Permissão</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-purple-300">Membro desde</th>
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-purple-300">Ações</th>
                    </tr>
                </thead>
                        ...
                        <tr class="group transition hover:bg-slate-800/30">
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-3">
                                    <div class="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500 to-blue-500 font-bold text-white shadow-lg shadow-purple-500/30 transition group-hover:scale-110 group-hover:shadow-purple-500/50">
                                    ...
                                    <div>
                                        <p class="font-semibold text-white group-hover:text-purple-300 transition">{{ $user->name }}</p>
                                        @if($user->id === auth()->id())
                            ...
                            <td class="px-6 py-5">
                                @if(auth()->user()->role === 'admin')
                                    <form action="{{ route('manage.users.update', $user) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PUT')
                                        <select name="role" onchange="this.form.submit()" class="rounded-lg border border-slate-700/50 bg-slate-950/50 backdrop-blur-sm px-4 py-2 text-sm font-semibold text-slate-50 transition focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 cursor-pointer hover:border-purple-500/50">
                                            <option value="cliente" {{ $user->role === 'cliente' ? 'selected' : '' }}>👤 Cliente</option>
                                            <option value="gestor" {{ $user->role === 'gestor' ? 'selected' : '' }}>⚙️ Gestor</option>
                                ...
                                @else
                                    @php
                                        $roleColors = [
                                            ...
                                            'gestor' => 'from-purple-500/20 to-blue-500/20 border-purple-500/50 text-purple-300',
                                            'cliente' => 'from-slate-700/20 to-slate-600/20 border-slate-600/50 text-slate-300'
                                        ];
                                        $roleIcons = ['admin' => '👑', 'gestor' => '⚙️', 'cliente' => '👤'];
                                    @endphp
                            ...
                            <td class="px-6 py-5 text-right">
                                @if($user->id !== auth()->id())
                                    <form action="{{ route('manage.users.destroy', $user) }}" method="POST" onsubmit="return confirm('⚠️ Tem a certeza que deseja eliminar {{ $user->name }}?\n\nEsta ação é irreversível.');" class="inline-block">
                                        @csrf
                                        @method('DELETE')
