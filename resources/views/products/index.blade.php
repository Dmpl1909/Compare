...
<div class="space-y-8">
    ...
    <section class="space-y-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-4xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 to-teal-400 bg-clip-text text-transparent">Catálogo de Jogos</h1>
        ...
        <form method="GET" action="{{ route('products.catalog') }}" class="relative group">
            ...
            <div class="relative">
                ...
                <input id="q" name="q" value="{{ $term }}" placeholder="Pesquisar por título, editora ou SKU..." 
                    class="w-full rounded-xl border border-slate-800/50 bg-slate-900/50 backdrop-blur-sm py-4 pl-12 pr-32 text-slate-50 placeholder-slate-500 transition focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20" />
                <button type="submit" class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg bg-gradient-to-r from-cyan-600 to-teal-600 px-6 py-2.5 font-semibold text-white transition hover:from-cyan-500 hover:to-teal-500 shadow-lg shadow-cyan-500/30">
                    Procurar
                </button>
            </div>
        </form>
    ...
    <section class="space-y-6">
        @forelse ($products as $product)
            ...
            @endphp
            <article class="group relative overflow-hidden rounded-2xl border border-slate-800/50 bg-gradient-to-br from-slate-900/50 to-slate-900/80 backdrop-blur-sm transition hover:border-cyan-500/50 hover:shadow-xl hover:shadow-cyan-500/10">
                <div class="absolute inset-0 bg-gradient-to-br from-cyan-600/0 to-teal-600/0 opacity-0 transition group-hover:from-cyan-600/5 group-hover:to-teal-600/5 group-hover:opacity-100"></div>
                    ...
                    <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                        ...
                        <div class="flex-1 space-y-4">
                            <div class="flex gap-4">
                                ...
                                <div class="space-y-2 flex-1">
                                    <h2 class="text-2xl font-bold group-hover:text-cyan-300 transition">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($product->offers as $offer)
                                    <span class="group/pill inline-flex items-center gap-2 rounded-full border border-slate-700/50 bg-slate-800/30 px-3 py-1.5 text-xs transition hover:border-cyan-500/50 hover:bg-slate-800/50">
                        ...
                        <div class="flex flex-col items-start md:items-end gap-4 md:min-w-[240px]">
                            ...
                            <div class="flex w-full flex-col gap-2">
                                ...
                                @else
                                    <a href="{{ route('login') }}" class="w-full rounded-lg bg-gradient-to-r from-cyan-600 to-teal-600 px-4 py-2.5 text-center text-sm font-bold text-white transition hover:from-cyan-500 hover:to-teal-500 shadow-lg shadow-cyan-500/30">
